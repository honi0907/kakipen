...
public static class HostSettingsWindowHelper
{
    private static HostSettingsWindow? _window;
    private static Func<Task>? _liveApplyAsync;
    private static Func<Task>? _refreshAssetsAsync;

    public static void ShowOrActivate(
        Func<Task>? onLiveApplyAsync = null,
        Func<Task>? onRefreshAssetsAsync = null)
    {
        _liveApplyAsync = onLiveApplyAsync;
        _refreshAssetsAsync = onRefreshAssetsAsync;
    ...
    private static void ShowOrActivateCore()
    {
        try
        {
            ...
            _window = new HostSettingsWindow(_liveApplyAsync, _refreshAssetsAsync);
            _window.Closed += (_, _) => _window = null;
            _window.Activate();
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"[HostSettingsWindowHelper] failed: {ex}");
            _window = null;
