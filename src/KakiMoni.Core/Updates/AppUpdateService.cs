...
public sealed class AppUpdateService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var assetName = Path.GetFileName(check.AssetName ?? "KakiMoni-Update.exe");
        var targetPath = Path.Combine(
            Path.GetTempPath(),
            $"kakimoni-update-{DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()}-{assetName}");
        ...
        CancellationToken cancellationToken)
    {
        ...
        var candidates = releases
            ...
            .Where(r => r.TagName!.StartsWith(profile.ReleaseTagPrefix, StringComparison.OrdinalIgnoreCase)
                        || r.TagName.StartsWith(profile.ReleaseTagPrefix.TrimEnd('v', 'V'), StringComparison.OrdinalIgnoreCase))
            .Select(r => new
            {
                Release = r,
