...
public sealed class OnlineUpdateCoordinator
{
    ...
    {
        var profile = AppReleaseProfile.For(kind);
        var current = AppVersionReader.GetCurrentVersion();
        var token = githubToken ?? Environment.GetEnvironmentVariable("KAKIMONI_GITHUB_TOKEN");
        ...
        try
        {
            ...
            if (!PackagedAppDetector.CanApplyOnlineUpdate())
            {
                return OnlineUpdateRunResult.Failed(
                    "開発ビルドでは自己更新を実行できません。dist/ またはインストール済みの exe から起動してください。");
            }
